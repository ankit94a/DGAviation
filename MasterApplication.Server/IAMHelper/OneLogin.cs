...
/// </summary>
namespace MasterApplication.Server.IAMHelper
{
    namespace OneLogin
    {
        namespace Saml
        {
            ...
            public class Response
            {
                ...
                public void LoadXml(string xml)
                {
                    ...
                    MasterLogger.Warn(xml, "ERROR-LOADING-XML", "-----eNDING");
                    xmlDoc.LoadXml(xml);
                }
                ...
                public void LoadXmlFromBase64(string response)
                {
                    if (response != null)// anil
                    {
                        ...
                        MasterLogger.Warn(response, "BASE64CONVERTED", "-----eNDING");
                ...
                {
                    bool status = true;
                    MasterLogger.Warn(status.ToString(), "STATUSTRUE", "-----eNDING");
                    if (xmlDoc != null)
                    {
                        #region signatureVerification
                        SignatureVerification ss = new SignatureVerification();
                        //string cert = Directory.GetCurrentDirectory();
                       
                        string cert = AppDomain.CurrentDomain.BaseDirectory;
                        string certPathKey = Path.Combine(cert, "Certs", "class3.cer");
                        
                        //string path = @"C:\\Certs\\App Certificate\\iambcpc.amy.mil.p12";
                        string result = ss.ValidateSignature(xmlDoc.DocumentElement, certPathKey, "emudhra");
                        MasterLogger.Warn(result.ToString(), "RESULTTRUEORFALSE", "-----eNDING");
                        if (result == "True")
                        {
